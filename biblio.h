...
bool FctOuvertureFichiers(void);
...
bool ouvertureFichiers(void) {
	...
	// Fichiers Livres
	FILE* pTabLivres;
	fopen_s(&pTabLivres, NOM_FICHIER_OUVRAGES, "rb+");
	if (pTabLivres == NULL) {
...
Livre obtenirLivre(char isbn[]) {
    ...
    // Initialisation d'un livre "invalide"
    memset(&livreBD, 0, sizeof(livreBD));
    strcpy_s(livreBD.isbn, TAILLE_CODE, "");  // ISBN vide pour indiquer une erreur
    ...
    if (feof(pTabLivres)) {
        fclose(pTabLivres);
        memset(&livreBD, 0, sizeof(livreBD));
        strcpy_s(livreBD.isbn, TAILLE_CODE, "");
        return livreBD;
    }
